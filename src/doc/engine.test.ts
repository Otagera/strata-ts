...
test("StrataDoc: Find & Cursor", async () => {
  ...
  const alice = await db.find("users", { name: "Alice", dept: "Eng" }).toArray();
...
test("StrataDoc: Advanced Operators", async () => {
  ...
  // 1. $gt / $lt
  const thirties = await db.find("users", { age: { $gt: 25, $lt: 40 } }).toArray();
...
test("StrataDoc: Secondary Indexing", async () => {
  ...
  const indexKeys = [];
  for await (const { key } of kv.scan("IDX")) { // Check unencoded prefix logic?
