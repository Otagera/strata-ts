...
describe("WAL Performance Benchmark", () => {
    ...
    test("Compare throughput with and without WAL", async () => {
        ...
        // 2. Without WAL
        const dbNoWal = new StrataKV({ dataDir: PERF_DIR_NO_WAL, walEnabled: false });
        ...
        console.log(`With WAL:    ${timeWal.toFixed(2)}ms (${(COUNT / timeWal * 1000).toFixed(0)} ops/sec)`);
        console.log(`Without WAL: ${timeNoWal.toFixed(2)}ms (${(COUNT / timeNoWal * 1000).toFixed(0)} ops/sec)`);
        console.log(`Overhead:    ${((timeWal - timeNoWal) / timeNoWal * 100).toFixed(1)}% slower`);
        console.log(`-------------------------------------------\n`);
    ...
    test("Compare throughput with High Memtable Limit", async () => {
        ...
        console.log(`With WAL (Disk):  ${timeWal.toFixed(2)}ms (${(COUNT / timeWal * 1000).toFixed(0)} ops/sec)`);
        console.log(`No WAL (Memory):  ${timeNoWal.toFixed(2)}ms (${(COUNT / timeNoWal * 1000).toFixed(0)} ops/sec)`);
        console.log(`Overhead:         ${((timeWal - timeNoWal) / timeNoWal * 100).toFixed(1)}% slower`);
        console.log(`----------------------------------------------------------\n`);
    });
});
